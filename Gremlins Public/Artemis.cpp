...
{
	HMODULE hKernel32 = GetModuleHandleA(es("kernel32.dll"));
	if (!hKernel32)
		return false;

	FARPROC pIsDebuggerPresent = GetProcAddress(hKernel32, es("IsDebuggerPresent"));
...
{
	HMODULE hNtdll = GetModuleHandleA(es("ntdll.dll"));
	if (!hNtdll)
		return;

	FARPROC pDbgBreakPoint = GetProcAddress(hNtdll, es("DbgBreakPoint"));
	if (!pDbgBreakPoint)
		return;
...
void CArtemis::PatchRemoteBreakin()
{
	...
	FARPROC pDbgUiRemoteBreakin = GetProcAddress(hNtdll, es("DbgUiRemoteBreakin"));
	...
	FARPROC pTerminateProcess = GetProcAddress(hKernel32, es("TerminateProcess"));
...
bool CArtemis::NtQuerySystemInfoCheck()
{
	...
	FARPROC _instr = GetProcAddress(hNtdll, es("NtQuerySystemInformation"));
