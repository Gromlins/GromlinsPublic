...
bool ImGui::tab(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(50, 40), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    float deltatime = 3.5f * ImGui::GetIO().DeltaTime;

    const ImVec4 text_dis = ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
    ...
    if (selected)
    {
        draw->AddRectFilled(bb.Min, bb.Max, ImColor(255, 255, 255));
        draw->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 2, bb.Max.y), ImColor(255, 255, 255));
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(12, 13, 13, 255), 3.f);

    window->DrawList->AddRectFilled(rect.Min, rect.Max,  ImColor(44, 15, 15, it_anim->second.active_rect_alpha), 4.f);

    window->DrawList->AddRect(rect.Min, rect.Max, ImColor(20, 20, 20, (15 + it_anim->second.active_outline_anim)), 4.f);

    //PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(255, 0, 0, 255)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(ImVec2(window->DC.CursorPos.x - 0, window->DC.CursorPos.y - 0), window->DC.CursorPos - ImVec2(5, 5) + ImVec2(label_size.y + style.FramePadding.y * 2, label_size.y + style.FramePadding.y * 2));
    ...
    if (*v)
        it_hover->second = ImClamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * (*v ? 1.f : -1.f)), 1.0f, 1.f);
    else
        it_hover->second = ImClamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime), 1.0f, 1.f);

    const ImVec4 hover_dis = ImColor(135, 88, 245, 255);
    const ImVec4 hover_act = ImColor(231, 231, 231, 255);
    ...
    {
        ImVec4 to = *v ? hover_dis : hover_act;
        if (it_hcolor->second.x != to.x)
        {
            if (it_hcolor->second.x < to.x)
                it_hcolor->second.x = ImMin(it_hcolor->second.x + deltatime, to.x);
    ...
    RenderFrame(ImVec2(check_bb.Min.x, check_bb.Min.y - 1), ImVec2(check_bb.Max.x, check_bb.Max.y - 1), GetColorU32(ImVec4(it_hcolor->second.x, it_hcolor->second.y, it_hcolor->second.z, it_hover->second)), true, 2.5f);
    

    if (*v)
        RenderCheckMark(window->DrawList, ImVec2(check_bb.Min.x + 3, check_bb.Min.y), ImColor(255, 255, 255), 9);

    //ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0, 0, 0, 255));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Min.x + 25, check_bb.Min.y - 2), label);
    //ImGui::PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(11, 11, 11), style.ComboRounding, popup_open ? 3 : 15);
    ...
    if (label_size.x > 0)
        RenderComboText(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y - 20), ImColor(182, 152, 255), label);
...
static bool Items_ArrayGetter_string(std::vector<std::string> data, int idx, const char** out_text)
{
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 12), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 12), ImColor(231, 231, 231), 3);
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 12), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 12), ImColor(135, 88, 245), 3);
    

    // Render grab
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 5, grab_bb.Min.y + 10), 6, ImColor(135, 88, 245), 360);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    
    //ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(44 / 255.f, 176 / 255.f, 129 / 255.f, 255));
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(135, 88, 245, 255));
    ImGui::PushFont(info);
    RenderTextClipped(ImVec2(frame_bb.Max.x + 50, frame_bb.Min.y), ImVec2(frame_bb.Max.x + 80, frame_bb.Max.y + 2), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    ImGui::PopFont();
    ImGui::PopStyleColor();

    if (label_size.x > 0.0f)
    {
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1));
...
bool ImGui::InputTextNew(const char* label, char* buf, size_t buf_size, ImVec2 Size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding, 0);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    if (!is_multiline)
    {
        ...
        //draw->AddRect(frame_bb.Min, frame_bb.Max, ImColor(255, 30, 30), 2);
    }
...
bool ImGui::sub(const char* label, bool selected)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(ImVec2(90, 50), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    const ImVec4 text_dis = ImVec4(115 / 255.f, 133 / 255.f, 157 / 255.f, 1.f);
    const ImVec4 text_act = ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
    ...
    if (selected)
    {
        draw->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y), ImVec2(bb.Max.x, bb.Max.y - 1), ImColor(255, 0, 0));
