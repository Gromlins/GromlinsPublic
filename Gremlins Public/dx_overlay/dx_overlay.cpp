...
namespace forceinline {
	...
	*/
	dx_overlay::dx_overlay(HWND target_hwnd, bool not_topmost) {
	...
	void dx_overlay::create_overlay(HWND target_hwnd) {
		WNDCLASSEX wc;
		wc.cbSize = sizeof(wc);
		...
		if (!RegisterClassExA(&wc))
			std::cout << "dx_overlay::create_overlay: failed to register wndclassex" << std::endl;

		//Find our target window
		m_target_wnd = target_hwnd;
		...
		if (!m_overlay_wnd)
			std::cout << "dx_overlay::create_overlay: failed to create overlay window" << std::endl;
	...
	void dx_overlay::init_dx9() {
		...
		if (!m_d3d)
			std::cout << "dx_overlay::init_dx9: failed to create dx3d9 object" << std::endl;
		...
		if (FAILED(m_d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_overlay_wnd, D3DCREATE_HARDWARE_VERTEXPROCESSING, &d3d_pp, &m_device))) {
			m_d3d->Release();
			std::cout << "dx_overlay::init_dx9: failed to create device" << std::endl;
		}
